...
class Products extends Component {
  render() {
    ...
    const produtos = data.map((products) => {
      return (
        <div className="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
          <a href="#">
            <img className="hover:grow hover:shadow-lg" src={products.img} />
            <div className="pt-3 flex items-center justify-between">
              <p className>{products.nome}</p>
              <svg
            ...
            <p className="pt-1 text-gray-900">{products.preco}</p>
          </a>
        </div>
      );
    });
